...
namespace Dapper.Extension.Core.Expressions
{
    public class WhereExpression: ExpressionVisitor
    {
        ...
        #region 增加处理参数重复代码
        private IDictionary<string, string> fieldDic = new Dictionary<string, string>();
        private int count = 0;
        #endregion
        ...
        protected override System.Linq.Expressions.Expression VisitMember(MemberExpression node)
        {
            ...
            #region 增加处理重复参数代码
            var flag = fieldDic.Keys.Contains(TempFieldName);
            if (flag)
                TempFieldName = $"{TempFieldName}_{count++}";
            fieldDic.TryAdd(TempFieldName, "2");
            #endregion
            return node;
        }

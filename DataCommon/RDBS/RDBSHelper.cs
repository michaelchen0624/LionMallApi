...
namespace DataCommon
{
    public class RDBSHelper
    {
        ...
        private static DbProviderFactory _factory=null;//抽象数据工厂

        private static DbProviderFactory _sqlfactory;
        private static DbProviderFactory _mysqlfactory;

        private static string _rdbstablepre = string.Empty;//关系数据库对象前缀
        private static string _connectionstring = string.Empty;//关系数据库连接字符串
        ...
        static RDBSHelper()
        {
            _sqlfactory = SqlClientFactory.Instance;

            //_rdbstablepre = JcConfig.RDBSConfig.RDBSTablePre;
            //_connectionstring = JcConfig.RDBSConfig.RDBSConnectString;
            _mysqlfactory = MySqlClientFactory.Instance;
        }
        //public static IDbTransaction GetDbTran(string database)
        //{
        //    var conn = GetDbConn(database);
        ...
        /// 返回一个数据库连接
        /// </summary>
        public static IDbConnection GetDbConn(string database)
        {
            IDbConnection dbconnection = null;
            switch (database.ToLower())
            {
                case "sqlserver":
                    dbconnection = _sqlfactory.CreateConnection();
                    break;
                case "mysql":
                    dbconnection = _mysqlfactory.CreateConnection();
                    break;
                default:
                    dbconnection = _sqlfactory.CreateConnection();
                    break;
            ...
            //return _sqlfactory.CreateConnection();
            return dbconnection;
        }
        /// <summary>
        /// 关系数据库对象前缀
